...
export default function EditWeddingInvitationPage() {
    ...
    const fetchWeddingInvitation = async () => {
      try {
        ...
        const response = await axios.get(
          `${API_ENDPOINTS.weddingInvitations}/${params.id}`
        );
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      toast.error(
        error.response?.data?.message || "Lỗi khi cập nhật thiệp cưới"
      );
    } finally {
      setIsSaving(false);
    }
        ...
        <div className="flex items-center justify-between mb-8">
          <div className="flex items-center gap-4">
            <Button
              variant="outline"
              size="icon"
              onClick={() => router.push("/admin/wedding-invitations")}
              className="h-9 w-9"
            >
          ...
          <div className="space-y-4">
            ...
            <div className="space-y-4">
              <div className="space-y-2">
                ...
                <Select
                  value={weddingInvitation.status}
